...
def test_calculate_mortgage_payment_zero_inputs():
    ...
    assert calculate_mortgage_payment(250000, 0, 25) == pytest.approx(833.3333333333)
...
def test_calculate_forecast_higher_rate_sipp_relief_adds_extra_cash_relief():
    result = calculate_forecast(
        income=5000,
        expenses=2000,
        isa_assets=0,
        ...
        sipp_contribution=100,
        sipp_rate=0,
        pension_tax_relief_rate=40,
        pension_interest_rate=0,
    )

    # Higher-rate extra relief is modelled as cash reclaimed via tax return.
    assert result["monthly_savings"] == pytest.approx(2920.0)
    # Pension top-up remains relief-at-source (basic-rate at provider level).
    assert result["pension_values"][-1] == pytest.approx(10125.0)
...
def test_calculate_forecast_total_gain_uses_full_initial_wealth_baseline():
    result = calculate_forecast(
        income=0,
        expenses=0,
        isa_assets=10000,
